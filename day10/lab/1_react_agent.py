...
Rules:
- NEVER generate "Observation:" yourself. Stop generating immediately after the "Input: ..." line.
...
def run_react_agent(question: str) -> dict:
    ...
    # Rebuild system prompt dynamically to ensure registered tools are current
    system_prompt = f"""You are a data engineering agent for Sigma DataTech.
You answer questions by reasoning step-by-step and using tools.
...
- Maximum {MAX_ITER} steps then give your best answer.
- NEVER make up data.
- If the user asks you to flag merchants, you MUST call flag_merchant for each merchant before giving a Final Answer.
- Do not claim a merchant was flagged unless flag_merchant returned success.
"""
...
""")
    ...
    def tool_flag_merchant(input_str: str) -> str:
        ...
        """
        # Split on the first comma to get merchant_id and reason
        parts = input_str.split(",", 1)
        merchant_id = parts[0].strip()
        reason = parts[1].strip() if len(parts) > 1 else "No reason provided"

        # Load existing flagged merchants list (or start fresh)
    ...
    # Already added to TOOL_DESCRIPTIONS near line 128 (see above).
    # That string feeds directly into the LLM system prompt so the agent knows
    # the tool exists and how to call it.

    flag_question = (
        "Find ALL merchants where transaction_count >= 1 AND avg_amount < 15. "
