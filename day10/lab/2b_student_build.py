...
def sql_checker_node(state: CheckerState) -> dict:
    ...
    check_reason = "SQL query contains a WHERE clause" if is_safe else "No WHERE clause found in SQL query"
...
def safe_executor_node(state: CheckerState) -> dict:
    ...
    if state["is_safe"]:
        try:
            ...
            result = conn.execute(state["sql"]).fetchdf()
            conn.close()
            if result.empty:
                return {"result": "Query returned 0 rows."}
            return {"result": result.to_string(index=False, max_rows=20)}
